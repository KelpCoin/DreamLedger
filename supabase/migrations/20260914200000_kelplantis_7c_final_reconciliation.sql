...
grant select on table public.kelplantis_player_inventory to anon,authenticated;
grant insert,update on table public.kelplantis_player_inventory to anon,authenticated;
...
begin
  select p.id into v_player_id from public.kelplantis_players as p where p.player_token=p_player_token;
  if v_player_id is null then raise exception 'UNKNOWN_PLAYER'; end if;
  insert into public.kelplantis_player_inventory(player_id) values(v_player_id)
  on conflict on constraint kelplantis_player_inventory_pkey do nothing;
  return query select i.player_id,i.kelp,i.gold,i.pearls from public.kelplantis_player_inventory as i where i.player_id=v_player_id;
end;
$$;
...
create trigger kelplantis_resource_nodes_broadcast after insert or delete or update on public.kelplantis_resource_nodes for each row execute function public.kelplantis_broadcast_resource_change();

alter table public.kelplantis_resource_nodes replica identity full;
do $$ begin alter publication supabase_realtime add table public.kelplantis_resource_nodes; exception when duplicate_object then null; end $$;

insert into public.kelplantis_resource_nodes(depth_id,resource_type,x,y,amount,max_amount)
select 1,'kelp',v.x,v.y,10,10 from (values (4,7),(9,6),(14,8),(26,7),(32,6),(36,10),(5,34),(10,36),(16,33),(25,35),(31,34),(36,31),(6,16),(12,18),(18,15),(24,18),(30,16),(35,20),(6,24),(12,25),(18,27),(24,25),(30,27),(35,24)) v(x,y)
where not exists(select 1 from public.kelplantis_resource_nodes n where n.depth_id=1 and n.resource_type='kelp' and n.x=v.x and n.y=v.y);
