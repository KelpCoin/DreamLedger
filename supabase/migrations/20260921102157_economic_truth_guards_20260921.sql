...
begin
 if tg_table_name='economic_outcomes' and coalesce(j->>'truth_status','')='VERIFIED' then
   ...
   if not(coalesce(j->>'scope','')='EXTERNAL' and coalesce(j->>'observation_mode','')='OBSERVED' and coalesce((j->>'buyer_action_verified')::boolean,false) and coalesce((j->>'payment_settled')::boolean,false) and coalesce((j->>'fulfilment_verified')::boolean,false) and coalesce((j->>'evidence_verified')::boolean,false) and nullif(j->>'stripe_payment_intent','') is not null and nullif(j->>'stripe_checkout_session','') is not null and nullif(j->>'evidence_ref','') is not null) then
