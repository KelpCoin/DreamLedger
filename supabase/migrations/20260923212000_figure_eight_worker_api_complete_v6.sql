...
-- v4 repair: keep the complete worker API surface and harden crossing immutability.

create or replace function figure_eight.reject_offer_promotion_mutation()
returns trigger
language plpgsql
as $$
begin
  raise exception 'FIGURE_EIGHT_APPEND_ONLY: offer_promotions is append-only';
end;
$$;

drop trigger if exists offer_promotions_append_only on figure_eight.offer_promotions;
create trigger offer_promotions_append_only
before update or delete on figure_eight.offer_promotions
for each row execute function figure_eight.reject_offer_promotion_mutation();
...
comment on function public.figure_eight_worker_api(text,text,jsonb) is 'Figure Eight complete worker API v6: v2 surface plus evidence, materialization, replication, SEO, and verified mechanism discovery.';
