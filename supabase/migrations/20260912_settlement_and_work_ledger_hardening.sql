...
create table if not exists public.processed_webhook_events (provider_event_id text primary key,payload jsonb not null,received_at timestamptz not null default now(),status text not null default 'pending' check(status in('pending','processing','done','failed')),lease_owner text,lease_expires_at timestamptz,attempt_count integer not null default 0 check(attempt_count>=0),started_at timestamptz,completed_at timestamptz,error text);
create index if not exists processed_webhook_events_drain_idx on public.processed_webhook_events(status,received_at);
create index if not exists processed_webhook_events_lease_idx on public.processed_webhook_events(status,lease_expires_at) where status='processing';
alter table public.processed_webhook_events enable row level security;
revoke all on public.processed_webhook_events from anon,authenticated;

create or replace function public.settlement_enqueue_webhook(p_event_id text,p_payload jsonb)
returns boolean language plpgsql security definer set search_path='' as $$
begin
  if p_event_id is null or p_payload is null then raise exception 'webhook event id and payload required' using errcode='P0001'; end if;
  insert into public.processed_webhook_events(provider_event_id,payload) values(p_event_id,p_payload) on conflict(provider_event_id) do nothing;
  return true;
end;
$$;
create or replace function public.settlement_claim_batch(p_worker_id text,p_max integer,p_lease_secs integer)
returns table(provider_event_id text,payload jsonb,attempt_count integer) language plpgsql security definer set search_path='' as $$
begin
  ...
  return query with candidates as (select p.provider_event_id from public.processed_webhook_events p where p.status='pending' or(p.status='processing' and p.lease_expires_at<=now()) order by p.received_at asc limit p_max for update skip locked)
  update public.processed_webhook_events p set status='processing',lease_owner=p_worker_id,lease_expires_at=now()+make_interval(secs=>p_lease_secs),attempt_count=p.attempt_count+1,started_at=coalesce(p.started_at,now()),error=null from candidates c where p.provider_event_id=c.provider_event_id returning p.provider_event_id,p.payload,p.attempt_count;
end;
$$;
create or replace function public.settlement_finish(p_event_id text,p_status text,p_error text) returns boolean language plpgsql security definer set search_path='' as $$
declare v_updated integer;begin if p_status not in('done','failed') then raise exception 'invalid settlement terminal status' using errcode='P0001';end if;update public.processed_webhook_events set status=p_status,completed_at=now(),error=p_error,lease_expires_at=null where provider_event_id=p_event_id and status='processing';get diagnostics v_updated=row_count;return v_updated=1;end;$$;
revoke execute on function public.settlement_enqueue_webhook(text,jsonb) from public,anon,authenticated;
...
grant execute on function public.settlement_enqueue_webhook(text,jsonb) to service_role;
...
begin
 ...
 if p_to_state='fulfillment' and(auth.role()<>'service_role' or p_actor_user_id is not null) then raise exception 'settlement authority required' using errcode='P0005';end if;
...
create or replace function public.claim_task(p_worker_id text,p_tier text,p_lease_secs integer default 30) returns table(task_id uuid,role text,objective text,input jsonb,lease_token uuid,attempt_count integer) language plpgsql security definer set search_path='' as $$begin return query select t.task_id,t.role,t.objective,t.input,t.lease_token,t.attempt_count from public.claim_orchestrator_task(p_tier,p_worker_id,p_lease_secs) t;end;$$;
create or replace function public.renew_task_lease_for_worker(p_task_id uuid,p_lease_token uuid,p_worker text,p_lease_secs integer default 30) returns boolean language sql security definer set search_path='' as $$select public.renew_orchestrator_task_lease(p_task_id,p_lease_token,p_worker,p_lease_secs);$$;
create or replace function public.finish_task_for_worker(p_task_id uuid,p_lease_token uuid,p_worker text,p_status text,p_result jsonb default null,p_error text default null) returns boolean language sql security definer set search_path='' as $$select exists(select 1 from public.finish_orchestrator_task(p_task_id,p_lease_token,p_worker,p_status,p_result,p_error));$$;
