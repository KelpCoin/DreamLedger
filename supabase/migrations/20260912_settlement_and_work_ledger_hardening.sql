...
begin
  if p_max < 1 or p_max > 100 then
    raise exception 'invalid batch size' using errcode = 'P0001';
  end if;
  if p_lease_secs < 5 or p_lease_secs > 3600 then
    raise exception 'invalid lease seconds' using errcode = 'P0001';
  end if;
...
create or replace function public.settlement_finish(
  p_event_id text,
  p_status text,
  p_error text
) returns boolean
...
begin
  if p_status not in ('done','failed') then
    raise exception 'invalid settlement terminal status' using errcode = 'P0001';
  end if;
...
-- The existing marketplace schema uses order_state/state_version rather than status/version.
-- Keep that contract canonical and enforce the settlement authority at the database boundary.
...
begin
  ...
  if p_idempotency_key is not null and exists (
    select 1 from public.marketplace_order_state_events e
    where e.order_id = p_order_id and e.idempotency_key = p_idempotency_key
  ) then
    select * into r from public.marketplace_orders where id = p_order_id;
    return r;
  end if;

  select order_state, state_version, buyer_organization_id, seller_organization_id
  into old_state, current_version, buyer_org, seller_org
  ...
  if current_version <> p_expected_version then
    raise exception 'stale order version expected=% actual=%', p_expected_version, current_version using errcode = 'P0002';
  end if;

  if old_state = p_to_state then
    raise exception 'duplicate order transition without idempotency key' using errcode = 'P0003';
  end if;

  if p_to_state = 'fulfillment' and p_actor_user_id is null then
    -- A null actor is reserved for the server-side settlement consumer.
    if auth.role() <> 'service_role' then
      raise exception 'settlement authority required' using errcode = 'P0005';
    end if;
  elsif p_to_state = 'fulfillment' and auth.role() <> 'service_role' then
    raise exception 'settlement authority required' using errcode = 'P0005';
  end if;
  ...
  if auth.role() <> 'service_role' then
    ...
    if not exists (
      select 1 from public.marketplace_memberships m
      where m.organization_id = p_actor_org_id
        and m.user_id = p_actor_user_id
        and m.status = 'active'
        and m.role in ('owner','admin','buyer','seller','moderator','operator')
    ) then
  ...
  set order_state = p_to_state,
      ...
      state_updated_by = p_actor_user_id
  where id = p_order_id
    and state_version = p_expected_version
  returning * into r;

  if not found then
    raise exception 'concurrent modification' using errcode = 'P0002';
  end if;

  insert into public.marketplace_order_state_events(
    order_id, from_state, to_state, state_version,
    actor_user_id, actor_organization_id, idempotency_key, reason
  ) values (
    p_order_id, old_state, p_to_state, r.state_version,
    p_actor_user_id, p_actor_org_id, p_idempotency_key, p_reason
  );
...
-- Compatibility RPC names used by the local worker. They delegate to the canonical work-ledger functions.
create or replace function public.claim_task(
  p_worker_id text,
  p_tier text,
  p_lease_secs integer default 30
) returns table (
  task_id uuid,
  role text,
  objective text,
...
begin
  return query
  select t.task_id, t.role, t.objective, t.input, t.lease_token, t.attempt_count
  from public.claim_orchestrator_task(p_tier, p_worker_id, p_lease_secs) t;
end;
$$;
...
set search_path = ''
as $$
  select public.renew_orchestrator_task_lease(p_task_id,p_lease_token,current_setting('request.jwt.claim.sub', true),p_lease_secs);
$$;

-- The PowerShell worker supplies its worker id explicitly through the canonical function.
create or replace function public.renew_task_lease_for_worker(
  p_task_id uuid,
  p_lease_token uuid,
  p_worker text,
  p_lease_secs integer default 30
...
as $$
  select exists(
    select 1 from public.finish_orchestrator_task(
      p_task_id,p_lease_token,current_setting('request.jwt.claim.sub', true),p_status,p_result,p_error
    )
  );
$$;

create or replace function public.finish_task_for_worker(
  p_task_id uuid,
  p_lease_token uuid,
  p_worker text,
  p_status text,
...
as $$
  select exists(
    select 1 from public.finish_orchestrator_task(
      p_task_id,p_lease_token,p_worker,p_status,p_result,p_error
    )
  );
$$;

revoke execute on function public.claim_task(text,text,integer) from public, anon, authenticated;
revoke execute on function public.renew_task_lease(uuid,uuid,integer) from public, anon, authenticated;
revoke execute on function public.renew_task_lease_for_worker(uuid,uuid,text,integer) from public, anon, authenticated;
revoke execute on function public.finish_task(uuid,uuid,text,jsonb,text) from public, anon, authenticated;
...
grant execute on function public.renew_task_lease(uuid,uuid,integer) to service_role;
grant execute on function public.renew_task_lease_for_worker(uuid,uuid,text,integer) to service_role;
grant execute on function public.finish_task(uuid,uuid,text,jsonb,text) to service_role;
