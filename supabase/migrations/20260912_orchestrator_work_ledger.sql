...
create or replace function public.claim_orchestrator_task(p_tier text, p_worker text, p_lease_seconds integer default 30)
...
as $$
  ...
  where task_id = (
    select task_id from public.orchestrator_tasks
    where tier=p_tier and status='queued'
...
create or replace function public.renew_orchestrator_task_lease(p_task_id uuid, p_lease_token uuid, p_worker text, p_lease_seconds integer default 30)
returns boolean
language sql
security definer
set search_path = public
...
create or replace function public.finish_orchestrator_task(p_task_id uuid, p_lease_token uuid, p_worker text, p_status text, p_result jsonb default null, p_error text default null)
returns setof public.orchestrator_tasks
language sql
security definer
set search_path = public
as $$
  ...
  set status=p_status,
      result=p_result,
      error=p_error,
      completed_at=case when p_status in ('done','failed','timeout','cancelled') then now() else completed_at end,
