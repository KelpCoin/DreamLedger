...
begin
  ...
  e:=r.current_encounter;
  if e is null then raise exception 'no active encounter'; end if;
  roll:=('x'||substr(md5(coalesce(e->>'enemy_key','')||':'||coalesce(e->>'room_index','0')||':'||coalesce(e->>'enemy_hp','0')||':'||r.atk||':player'),1,8))::bit(32)::bigint;
  pd:=3+mod(abs(roll),greatest(r.atk,1));
  neh:=(e->>'enemy_hp')::int-pd;
  insert into public.kelplantis_events(player_token,event_type,payload) values(p_token,'damage_dealt',jsonb_build_object('source','player','amount',pd,'target',e->>'enemy_name','deterministic',true));
  if neh<=0 then
    d:=r.dungeon_state;
    i:=(d->>'room_index')::int;
    d:=jsonb_set(d,array['rooms',i::text,'cleared'],'true'::jsonb);
    nx:=r.xp; nl:=r.level; inv:=r.inventory; fp:=coalesce(r.floor_progress,'{}'::jsonb); qs:=coalesce(r.quest_state,'{}'::jsonb);
    if e->>'type'='boss' then
      firstclear:=not coalesce((fp->'1'->>'bossDefeated')::boolean,false);
      nx:=nx+40;
      fp:=jsonb_set(fp,'{1}',coalesce(fp->'1','{}'::jsonb)||jsonb_build_object('bossDefeated',true,'clearedAt',now()),true);
      roll:=('x'||substr(md5('floor1:boss:loot'),1,8))::bit(32)::bigint;
      loot:=lt->mod(abs(roll),jsonb_array_length(lt))::int;
      loot:=loot||jsonb_build_object('id',(loot->>'key')||'_floor1_boss');
      inv:=inv||jsonb_build_array(loot);
      ...
      if qs->>'garden_warden_intro'='accepted' then
        ...
        nx:=nx+coalesce(qr,0);
        qs:=jsonb_set(qs,'{garden_warden_intro}','"completed"'::jsonb,true);
      ...
      update public.kelplantis_players set scene='TOWN',pos_x=2,pos_y=1,dungeon_state=null,current_encounter=null,xp=nx,level=nl,max_hp=24+(nl-1)*6,atk=5+(nl-1)*2,hp=least(r.hp,24+(nl-1)*6),inventory=inv,floor_progress=fp,quest_state=qs,updated_at=now() where player_token=p_token returning * into r;
      if firstclear then
      ...
      update public.kelplantis_players set dungeon_state=d,current_encounter=null,xp=nx,level=nl,max_hp=24+(nl-1)*6,atk=5+(nl-1)*2,hp=least(24+(nl-1)*6,r.hp+2),updated_at=now() where player_token=p_token returning * into r;
    end if;
  ...
  ed:=1+mod(abs(roll),greatest((e->>'enemy_atk')::int,1));
  nph:=r.hp-ed;
...
revoke execute on function public.kelplantis_record_floor1_boss_clear(uuid) from anon, authenticated;
revoke execute on function public.kelplantis_apply_floor1_first_clear(uuid) from anon, authenticated;
