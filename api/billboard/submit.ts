...
export default async function handler(request:Request){
 ...
 const email=String(body.email||'').trim();const title=String(body.title||'').trim();const owner_name=String(body.owner_name||'').trim();const destination_url=String(body.destination_url||'').trim();const image_url=String(body.image_url||'').trim();
