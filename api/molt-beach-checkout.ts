...
const OFFERS:any={
 '100x100':{sku:'BILLBOARD-SMALL',name:'DreamLedger Founding Tile - 100x100',amount:2900,w:100,h:100},
...
export default async function handler(request:Request){
 ...
 const form=new URLSearchParams();form.set('mode','payment');form.set('line_items[0][price_data][currency]','nzd');form.set('line_items[0][price_data][product_data][name]',offer.name);form.set('line_items[0][price_data][unit_amount]',String(offer.amount));form.set('line_items[0][quantity]','1');form.set('success_url','https://dreamledger.org/billboard?paid=1&session_id={CHECKOUT_SESSION_ID}');form.set('cancel_url','https://dreamledger.org/billboard?cancelled=1');
 form.set('metadata[molt_beach]','true');form.set('metadata[billboard]','true');form.set('metadata[sku]',offer.sku);form.set('metadata[x]',String(slot.x));form.set('metadata[y]',String(slot.y));form.set('metadata[width]',String(w));form.set('metadata[height]',String(h));
