...
if (!global.__dreamledgerHypothesisCommercePreload) {
 ...
 + h.price_nzd.toFixed(2) + '</a></p><p class="meta">Hypothesis ID: ' + esc(h.hypothesis_id) + '<br>Source basis: ' + esc(h.source_basis) + '<br>Payment attribution: Stripe client reference ID<br>Revenue truth: settled external payment only, followed by fulfillment evidence.</p><p class="meta"><a href="' + stripeUrlForHypothesis(h) + '" style="color:#f0c85a">Open Stripe payment link</a></p></section></main><script>document.getElementById("checkout").addEventListener("submit",async function(e){e.preventDefault();const status=document.getElementById("status");status.textContent="Opening secure Stripe Checkout...";try{const email=this.email.value;const r=await fetch("/api/hypothesis-checkout",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({hypothesis_id:' + JSON.stringify(h.hypothesis_id) + ',email:email})});const d=await r.json();if(!r.ok)throw new Error(d.error||"Checkout failed");location.href=d.checkout_url}catch(err){status.textContent=err.message||"Checkout failed"}});</script></body></html>';
}
