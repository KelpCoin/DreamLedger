...
const PUBLIC_FILES={'/c2':'c2.html','/c2/':'c2.html','/consult':'c2.html','/consult/':'c2.html','/':'index.html','/silos':'silos/index.html','/silos/':'silos/index.html','/silos/index.html':'silos/index.html','/truth-oracle':'truth-oracle.html','/truth-oracle/':'truth-oracle.html','/truth-oracle.html':'truth-oracle.html','/index.html':'index.html','/guides':'guides/index.html','/guides/':'guides/index.html','/agent-commerce':'agent-commerce.html','/agent-commerce/':'agent-commerce.html','/avatar.html':'avatar.html','/dreamiez':'avatar.html','/dreamiez/':'avatar.html','/marketplace':'cube-marketplace.html','/marketplace/':'cube-marketplace.html','/b2b':'cube-marketplace.html','/b2b/':'cube-marketplace.html','/billboard':'billboard.html','/billboard/':'billboard.html','/mtg':'mtg.html','/mtg/':'mtg.html','/mtg/commander-deck-diagnostic':'commander-deck-diagnostic.html','/mtg/commander-deck-diagnostic/':'commander-deck-diagnostic.html','/avatar':'avatar.html','/avatar/':'avatar.html','/avatars':'avatar.html','/avatars/':'avatar.html','/dreammeez':'avatar.html','/dreammeez/':'avatar.html','/dreamiez':'dreamiez.html','/dreamiez/':'dreamiez.html','/robots.txt':'robots.txt','/sitemap.xml':'sitemap.xml','/catalog.json':'catalog.json','/cube.json':'cube.json','/ecosystem.json':'ecosystem.json','/agent.json':'agent.json','/agent-commerce.json':'agent-commerce.json','/surfaces.json':'surfaces.json','/phin-haven':'phin-haven-v5.html','/phin-haven/':'phin-haven-v5.html','/phin-haven.html':'phin-haven-v5.html','/phin-haven-v5.html':'phin-haven-v5.html','/phinhaven':'phin-haven-v5.html','/phinhaven/':'phin-haven-v5.html','/phinhaven/town':'phinhaven/index.html','/phinhaven/town/':'phinhaven/index.html','/play':'phin-haven-v5.html','/play/':'phin-haven-v5.html','/overpaying':'overpaying.html','/overpaying/':'overpaying.html','/overpaying.html':'overpaying.html','/tools':'tools/index.html','/tools/':'tools/index.html','/tools/seller-profit':'tools/seller-profit/index.html','/tools/seller-profit/':'tools/seller-profit/index.html','/tools/seller-profit-audit':'tools/seller-profit-audit/index.html','/tools/seller-profit-audit/':'tools/seller-profit-audit/index.html','/tools/nz-seller-profit':'tools/nz-seller-profit/index.html','/tools/nz-seller-profit/':'tools/nz-seller-profit/index.html'};
...
const ALLOWED_API={'GET /api/molt-beach-inventory':true,'POST /api/seller-profit-lead':true,'GET /api/products':true,'GET /api/products/':true,'GET /api/offers':true,'GET /api/cube':true,'GET /api/ecosystem':true,'GET /api/agent':true,'GET /api/surfaces':true,'GET /api/billboard/submit':true,'POST /api/billboard/submit':true,'POST /api/offer-checkout/create':true,...ACCOUNT_API};
...
function localCubeMarketplace(){const products=loadPublicCatalog().products;return {items:products.filter(p=>p&&p.status==='published').map(p=>({source_silo:String(p.silo||'core'),title:String(p.name||p.title||p.id),description:String(p.description||''),price_nzd:Number(p.price||0),seller_name:String(p.seller_name||'DreamLedger')}))}}
...
async function cubeSiloPage(res,route){
  if(!SUPABASE_PUBLIC_KEY)return send(res,503,'CUBE route data unavailable','text/plain; charset=utf-8');
  ...
  const html='<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow"><link rel="canonical" href="https://dreamledger.org'+publicRoute+'"><title>'+title+' | DreamLedger CUBE</title><style>body{font-family:system-ui,-apple-system,sans-serif;background:#080a0d;color:#eef1f5;max-width:760px;margin:0 auto;padding:32px 20px}main{border:1px solid #343c4a;background:#141920;border-radius:16px;padding:24px}dt{color:#8a93a0;font-size:12px;margin-top:14px}dd{margin:4px 0}</style></head><body><main><p><a href="/">DreamLedger</a> / CUBE</p><h1>'+title+'</h1><p>DreamLedger CUBE economic silo.</p><dl><dt>Silo ID</dt><dd>'+id+'</dd><dt>Public route</dt><dd>'+publicRoute+'</dd><dt>Inventory mode</dt><dd>'+mode+'</dd></dl></main></body></html>';
  return send(res,200,html,'text/html; charset=utf-8');
}
function productSlug(p){return String(p.id||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}

function productPage(res,product){
  const title=String(product.name||product.id);
  const description=String(product.description||'');
  const price=Number(product.price||0);
  const currency=String(product.currency||'NZD').toUpperCase();
...
if(req.method==='GET'&&p==='/api/cube/catalog'){
  try{
    ...
    const total=slash>=0?Number(range.slice(slash+1)):null;
