...
const API_PREFIXES = ['/api/billboard'];
...
function headers(res) {
  ...
  res.setHeader('X-DreamLedger-Storefront', 'public-v2');
...
function isAllowedApi(p) {
  return API_PREFIXES.some(x => p === x || p.startsWith(x + '/'));
}
...
function proxy(req, res, body) {
  ...
  let target;
  try {
    target = new URL('http://' + ENGINE);
  } catch {
    return send(res, 502, 'Invalid engine address', 'text/plain; charset=utf-8');
  }
  ...
  upstream.on('error', () => {
    if (!res.writableEnded) send(res, 502, 'Service unavailable', 'text/plain; charset=utf-8');
  });
  upstream.end(body);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && p === '/version') {
    return send(res, 200, JSON.stringify({
      service: 'dreamledger-storefront',
      commit: COMMIT,
      surface: 'public-v2'
