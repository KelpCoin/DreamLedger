...
const PUBLIC_FILES={'/':'index.html','/index.html':'index.html','/billboard':'billboard.html','/billboard/':'billboard.html','/mtg':'mtg.html','/mtg/':'mtg.html','/avatar':'avatar.html','/avatar/':'avatar.html','/robots.txt':'robots.txt','/sitemap.xml':'sitemap.xml','/catalog.json':'catalog.json','/commerce.json':'commerce.json'};
const ALLOWED_API={'GET /api/molt-beach-inventory':true,'GET /api/products':true,'GET /api/products/':true,'POST /api/billboard/submit':true,'POST /api/offer-checkout/create':true};
...
function reserveBillboard(body){
if(!validText(body.title,120))throw new Error('Title or label is required');
if(!validText(body.name,120))throw new Error('Name or company is required');
if(!validText(body.email,200)||!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(body.email))throw new Error('Valid email is required');
if(!validUrl(body.link)||body.link.length>1000)throw new Error('Valid destination URL is required');
fs.mkdirSync(RESERVATION_DIR,{recursive:true});
const id='BB-'+new Date().toISOString().replace(/[-:.TZ]/g,'')+'-'+crypto.randomBytes(4).toString('hex');
const record={schema:'dreamledger/billboard-reservation/v1',reservation_id:id,product_id:'BILLBOARD-LARGE',offer_id:'OFFER-DREAMLEDGER-BILLBOARD-FOUNDING-001',title:body.title.trim(),name:body.name.trim(),email:body.email.trim(),link:body.link.trim(),price_nzd:50,payment_status:'pending',publication_status:'pending_payment_and_review',created_at:new Date().toISOString()};
fs.writeFileSync(path.join(RESERVATION_DIR,id+'.json'),JSON.stringify(record,null,2)+'\n','utf8');
return {ok:true,reservation_id:id,checkout_url:BILLBOARD_CHECKOUT+'?client_reference_id='+encodeURIComponent(id)+'&prefilled_email='+encodeURIComponent(record.email),payment_amount_nzd:50};
}
...
if(!STRIPE_WEBHOOK_SECRET||!header)throw new Error('Webhook verification not configured');
let timestamp=null;const signatures=[];for(const part of String(header).split(',')){const i=part.indexOf('=');if(i<0)continue;const k=part.slice(0,i),v=part.slice(i+1);if(k==='t')timestamp=v;if(k==='v1')signatures.push(v)}
if(!timestamp||!signatures.length)throw new Error('Invalid Stripe signature');
const age=Math.abs(Date.now()/1000-Number(timestamp));if(!Number.isFinite(age)||age>300)throw new Error('Expired Stripe signature');
const expected=crypto.createHmac('sha256',STRIPE_WEBHOOK_SECRET).update(timestamp+'.'+payload).digest('hex');
const ok=signatures.some(sig=>{try{return crypto.timingSafeEqual(Buffer.from(expected,'utf8'),Buffer.from(sig,'utf8'))}catch{return false}});
if(!ok)throw new Error('Invalid Stripe signature');
return true;
}
function proxy(req,res,body){if(!ENGINE||!ENGINE_KEY)return send(res,503,'Service unavailable','text/plain; charset=utf-8');let target;try{target=new URL('http://'+ENGINE)}catch{return send(res,502,'Invalid engine address','text/plain; charset=utf-8')}const up=http.request({hostname:target.hostname,port:Number(target.port||80),path:req.url,method:req.method,headers:{'x-dreamledger-internal-key':ENGINE_KEY,'content-type':req.headers['content-type']||'','content-length':body.length,'stripe-signature':req.headers['stripe-signature']||''}},r=>{res.statusCode=r.statusCode||502;for(const[k,v]of Object.entries(r.headers)){if(k!=='connection'&&k!=='transfer-encoding'&&v!==undefined)res.setHeader(k,v)}r.pipe(res)});up.setTimeout(20000,()=>up.destroy());up.on('error',()=>{if(!res.writableEnded)send(res,502,'Service unavailable','text/plain; charset=utf-8')});up.end(body)}
function serveFile(res,file){fs.readFile(path.join(ROOT,file),(err,data)=>{if(err)return send(res,404,'Not Found','text/plain; charset=utf-8');res.setHeader('Content-Type',MIME[path.extname(file).toLowerCase()]||'application/octet-stream');res.setHeader('Cache-Control','no-store');send(res,200,data)})}
...
if(req.method==='GET'&&p==='/version')return send(res,200,JSON.stringify({service:'dreamledger-storefront',commit:COMMIT,surface:'public-v6'}),'application/json; charset=utf-8');
...
if(req.method==='POST'&&p==='/api/billboard/submit'){let body;try{const raw=(await readBody(req)).toString('utf8');body=req.headers['content-type']&&req.headers['content-type'].toLowerCase().indexOf('application/json')>=0?JSON.parse(raw):null;if(!body)throw new Error('Use JSON submission');return send(res,200,JSON.stringify(reserveBillboard(body)),'application/json; charset=utf-8')}catch(e){return send(res,400,JSON.stringify({error:e&&e.message?e.message:'Invalid billboard submission'}),'application/json; charset=utf-8')}}
