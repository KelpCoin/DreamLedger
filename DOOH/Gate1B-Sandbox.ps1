...
function Invoke-Mcp {
    param([string]$Method,[object]$Params,[string]$Bearer='')
    ...
    foreach ($line in @($text -split "`n")) {
        if ($line -match '^data:\s*(.+)$') { try { return ($Matches[1] | ConvertFrom-Json) } catch {} }
    }
...
function Find-FirstValue {
    ...
    if ($Object -is [System.Collections.IDictionary]) {
        foreach ($name in $Names) { foreach ($key in $Object.Keys) { if ([string]$key -ieq $name) { return $Object[$key] } } }
        foreach ($key in $Object.Keys) { $found=Find-FirstValue -Object $Object[$key] -Names $Names; if ($null -ne $found) { return $found } }
    } elseif ($Object -is [System.Collections.IEnumerable] -and -not ($Object -is [string])) {
        foreach ($item in $Object) { $found=Find-FirstValue -Object $item -Names $Names; if ($null -ne $found) { return $found } }
    } else {
        foreach ($name in $Names) { $prop=$Object.PSObject.Properties | Where-Object { $_.Name -ieq $name } | Select-Object -First 1; if ($prop) { return $prop.Value } }
        foreach ($prop in $Object.PSObject.Properties) { $found=Find-FirstValue -Object $prop.Value -Names $Names; if ($null -ne $found) { return $found } }
    }
    return $null
}
...
if ([string]::IsNullOrWhiteSpace([string]$apiKey)) {
    $registration | ConvertTo-Json -Depth 50 | Set-Content -Path (Join-Path $OutputDir 'registration-response-no-key.json') -Encoding UTF8
    throw 'Agent registration completed without an API key. A diagnostic response without a key was persisted.'
}
...
$plan=[ordered]@{schema_version='DOOH-GATE1B-PLAN/1.0';generated_at=(Get-Date).ToUniversalTime().ToString('o');experiment='DOOH_CUBE_001';registration_tool=[string]$registerTool.name;inventory_tool=if($inventory){[string]$inventory.name}else{$null};products_tool=if($products){[string]$products.name}else{$null};create_media_buy_tool=if($createBuy){[string]$createBuy.name}else{$null};creative_sync_tool=if($creative){[string]$creative.name}else{$null};delivery_tool=if($delivery){[string]$delivery.name}else{$null};authority_boundary='No media-buy mutation is executed by this gate. First live booking remains human-authorized.'}
$plan | ConvertTo-Json -Depth 30 | Set-Content -Path (Join-Path $OutputDir 'gate1b-plan.json') -Encoding UTF8

$npx=Get-Command npx -ErrorAction SilentlyContinue
if ($npx -and @($screens).Count -eq 0) {
    ...
    $results.Add([pscustomobject]@{gate='GATE_1B';check='SANDBOX_CREATE';pass=$true;detail='CLI fallback used because registration response did not expose sandbox screens.'})
} elseif (@($screens).Count -gt 0) {
    $results.Add([pscustomobject]@{gate='GATE_1B';check='SANDBOX_CREATE';pass=$true;detail='Registration response supplied sandbox screens; CLI provisioning was skipped.'})
} else {
    $results.Add([pscustomobject]@{gate='GATE_1B';check='SANDBOX_CREATE';pass=$false;detail='npx unavailable and registration returned no sandbox screens.'})
}

$proof=[ordered]@{schema_version='DOOH-GATE1B-PROOF/1.1';generated_at=(Get-Date).ToUniversalTime().ToString('o');experiment='DOOH_CUBE_001';registration=[ordered]@{completed=$true;api_key_persisted=$false;sandbox_screens=@($screens).Count};authenticated_discovery=[ordered]@{completed=($authTools.Count -gt 0);observed_tool_count=$authTools.Count};sandbox=[ordered]@{create_attempted=($null -ne $npx -and @($screens).Count -eq 0);live_purchase=$false;live_booking=$false};media_buy=[ordered]@{executed=$false;reason='Human approval boundary and exact runtime tool arguments must be validated before mutation.'};cryptographic_proof=[ordered]@{real_proof_verified=$false;tamper_rejection_verified=$false};revenue=[ordered]@{verified_nzd=0;status='UNMATCHED'};next_gate='GATE_1B_MEDIA_BUY_PATH';results=@($results)}
