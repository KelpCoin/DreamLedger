...
foreach ($prod in $products) {
    ...
    Write-Host "Creating/verifying payment link for $skuId..."
...
if ($storeItems.Count -eq 0) {
    Write-Warning "No products with SKU metadata. The store will show a placeholder."
    $storeItems = @([pscustomobject]@{ Title = "No products yet"; SkuId = ""; Link = "#"; Sales = 0 })
}
$storeItems = $storeItems | Sort-Object Sales -Descending
$html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>DreamLedger Store</title><style>body{font-family:sans-serif;background:#0b0b0b;color:#e0e0e0;max-width:800px;margin:2rem auto}.card{background:#1a1a1a;border:1px solid #333;padding:1.2rem;margin-bottom:1.5rem;border-radius:8px}.card h2{color:#ffd966}.card p{color:#aaa}.buy{display:inline-block;background:#ffd966;color:#0b0b0b;font-weight:bold;padding:0.6rem 1.2rem;text-decoration:none;border-radius:6px}</style></head><body><h1 style="color:#ffd966">DreamLedger Bestsellers</h1><p>Instant delivery. Secure checkout via Stripe.</p>'
foreach ($it in $storeItems) {
    $badge = if ($it.Sales -gt 0) { " (Best Seller)" } else { "" }
    $html += "<div class='card'><h2>$($it.Title)$badge</h2><p>Sales: $($it.Sales)</p><a class='buy' href='$($it.Link)'>Buy Now - $5.00</a></div>"
}
