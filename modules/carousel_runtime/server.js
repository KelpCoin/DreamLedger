...
//  SIGNUP / LOGIN (simple file-based for now)
...
app.post('/signup', (req, res) => {
    ...
    users.push({ username, password }); // INSECURE  plain text for demo, replace with bcrypt later
    writeUsers(users);
    res.json({ ok: true });
});
...
app.get('/api/verify-invite', (req, res) => {
  ...
  res.json({ valid: true }); // Stub  no supabase for now
});
