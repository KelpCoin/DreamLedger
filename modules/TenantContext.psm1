...
function Set-TenantContext {
    param($TenantId)
    $script:CurrentTenant = $TenantId
    $env:TENANT_ID = $TenantId
}

function Get-TenantContext {
    return $script:CurrentTenant
}
...
function Get-GovernorPolicy {
    ...
    try {
        ...
        if ($response -and $response.Count -gt 0 -and $response[0].operational_tuning) {
            return $response[0].operational_tuning
        } else {
            Write-Warning "No active policy; using default."
            return (Get-Content "C:\BrownEyeCortex\DreamLedger\default_policy.json" | ConvertFrom-Json)
        }
    } catch {
        Write-Warning "Policy fetch failed: $_ ; using default."
        return (Get-Content "C:\BrownEyeCortex\DreamLedger\default_policy.json" | ConvertFrom-Json)
    }
}
