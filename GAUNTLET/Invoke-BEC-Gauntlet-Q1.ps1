...
foreach ($p in @($InboxPath,$CleanPath,$ReportsPath,$ProofPath,$LogsPath)) {
    Ensure-Directory $p
}
...
function Detect-Delimiter([string]$Path) {
    $sample = Get-Content -LiteralPath $Path -TotalCount 10 -Raw
...
function Process-Csv([string]$InputPath, [string]$OutputPath, [string]$ReportPath) {
    ...
    foreach ($row in $data) {
        ...
        foreach ($orig in $originalHeaders) {
            ...
            if ([string]::IsNullOrWhiteSpace($value)) {
                $anomalies.Add(("Row {0}: '{1}' is empty or whitespace" -f $rowIndex,$orig))
            }
    ...
    foreach ($row in $rows) {
        ...
        if ($seen.ContainsKey($key)) {
            $duplicates++
            $anomalies.Add(("Duplicate '{0}': {1}" -f $firstCol,$key))
        } else {
            $seen[$key] = $true
            $deduped.Add($row)
        }
    ...
    [ordered]@{
        ...
        SourceHash=(Get-Sha256 $InputPath)
...
if ($Action -eq 'Verify') {
    ...
    foreach ($sidecar in $sidecars) {
        ...
        $stored = (Get-Content -LiteralPath $sidecar.FullName -Raw).Trim().ToLowerInvariant()
        $actual = Get-Sha256 $json
...
if ($files.Count -eq 0) {
    ...
    if ($AllowEmptyInbox) {
        ...
        $proof = Write-Proof @{ run_type='Run'; status='NO_INPUT'; files_processed=0; files_failed=0; results=@() }
        exit 0
    }
...
foreach ($file in $files) {
    if ($file.Extension -match '^\.xls[x]?$') {
        ...
        if ($IsWindows -and (Get-Command excel.exe -ErrorAction SilentlyContinue)) {
            throw 'Excel conversion path is intentionally not enabled in the CI-safe Q1 runner.'
        }
    ...
    $clean = Join-Path $CleanPath ($base + '-clean.csv')
    $report = Join-Path $ReportsPath ($base + '-anomalies.md')
    try {
        $r = Process-Csv $file.FullName $clean $report
        if ($null -ne $r) { $results.Add($r) } else { $failed++ }
    } catch {
...
$rows = ($results | Measure-Object -Property RowCount -Sum).Sum
$dupes = ($results | Measure-Object -Property DuplicatesRemoved -Sum).Sum
$anoms = ($results | Measure-Object -Property AnomalyCount -Sum).Sum
...
Write-Host "ROWS CLEANED: $rows"
Write-Host "DUPLICATES REMOVED: $dupes"
Write-Host "ANOMALIES: $anoms"
