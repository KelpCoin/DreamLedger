...
const kelp7bEnter=document.getElementById('enter');kelp7bEnter.onclick=async()=>{try{state.name=document.getElementById('name').value.trim()||state.name;await kelp7bEnsurePlayer();await kelp7bResolveSpawn();await kelp7bSetOffline(false);persist();await kelp7bSyncLocal();refreshPresence();emit('movement',publicState());log('Entered Depth 1 from '+(state.x===20&&state.y===20?'the Fountain':'your Soul Tome anchor')+'.');}catch(e){log('7B login: '+e.message);}};
