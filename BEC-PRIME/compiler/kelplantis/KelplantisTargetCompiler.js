...
function run() {
  ...
  fs.writeFileSync(path.join(OUT, 'index.html'), buildRuntimeHtml(spec, dungeon), 'utf8');
  ...
  const proof = { schema:'bec/kelplantis-mvp-compiler-proof/v3', status:'PASS', base_universal_compile:base.status, target:'kelplantis-mvp', target_type:'game', runtime:'HTML5/browser, Windows-hostable', native_windows_exe:false, dungeon:{deterministic:true,generation_seed:dungeon.generation_seed,rooms:dungeon.rooms.length,corridors:dungeon.corridors.length,boss_arena:dungeon.boss_arena}, acceptance:{town:'SOURCE_PRESENT',combat:'SOURCE_PRESENT',ai_targeting:'SOURCE_PRESENT',loot:'SOURCE_PRESENT',leveling:'SOURCE_PRESENT',town_safety:'SOURCE_PRESENT',save_load:'SOURCE_PRESENT',launch:'GENERATED_NOT_RUNTIME_VERIFIED'}, outputs:files, runtime_verification:'NOT_EXECUTED' };
