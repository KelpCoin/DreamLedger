...
function canonicalGenerationRecord(input) {
  ...
  const version = String(input.generation_version || '1');
  const generationSeed = crypto.createHash('sha256').update(`${worldSeed}:${floorId}:${bossId}:${version}`, 'utf8').digest('hex');
  return {
    floor_id: floorId,
    generation_seed: generationSeed,
    generation_version: version,
    boss_id: bossId,
    first_generated_at_utc: input.first_generated_at_utc || null,
    historical_events: Array.isArray(input.historical_events) ? input.historical_events.slice() : [],
    canonical_boss_arena_locked: Boolean(input.canonical_boss_arena_locked)
  };
}
...
function appendHistoricalEvent(record, event) {
  ...
  next.historical_events.push({
    event_id: event.event_id || crypto.createHash('sha256').update(JSON.stringify(event), 'utf8').digest('hex').slice(0, 16),
