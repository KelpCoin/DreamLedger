...
function build7CScript() {
  return `
let kelp7cNodes=[],kelp7cJoined=false,kelp7cRealtimeUpdates=0,kelp7cInventory={kelp:0,gold:0,pearls:0};
async function kelp7cRest(path,options){const rt=G.realtime||{};const r=await fetch(rt.url+path,Object.assign({headers:{apikey:rt.publishableKey,Authorization:'Bearer '+rt.publishableKey,'Content-Type':'application/json'}},options||{}));const text=await r.text();let data=null;try{data=text?JSON.parse(text):null;}catch(_){data=text;}if(!r.ok)throw new Error((data&&data.message)||('7C request failed '+r.status));return data;}
async function kelp7cLoadNodes(){try{const data=await kelp7cRest('/rest/v1/kelplantis_resource_nodes?depth_id=eq.1&select=id,resource_type,x,y,amount,max_amount,respawn_at&order=y.asc,x.asc');kelp7cNodes=Array.isArray(data)?data:[];draw();return kelp7cNodes;}catch(e){log('7C nodes: '+e.message);return [];}}
async function kelp7cLoadInventory(){await kelp7bEnsurePlayer();const raw=await kelp7bRpc('kelplantis_get_inventory_for_token',{p_player_token:state.id});const row=Array.isArray(raw)?raw[0]:raw;if(row)kelp7cInventory={kelp:Number(row.kelp)||0,gold:Number(row.gold)||0,pearls:Number(row.pearls)||0};update7cInventoryUi();return kelp7cInventory;}
...
async function kelp7cHarvestNode(nodeId){await kelp7bEnsurePlayer();const node=kelp7cNodes.find(n=>n.id===nodeId);if(!node)throw new Error('RESOURCE_NODE_NOT_FOUND');const distance=Math.hypot(state.x-Number(node.x),state.y-Number(node.y));if(distance>2.5)throw new Error('TOO_FAR_FROM_RESOURCE_NODE');const raw=await kelp7bRpc('kelplantis_harvest_resource_for_token',{p_player_token:state.id,p_node_id:nodeId});const result=Array.isArray(raw)?raw[0]:raw;log('Harvested '+result.yield_amount+' '+result.resource_type+'.');await kelp7cLoadNodes();await kelp7cLoadInventory();return result;}
