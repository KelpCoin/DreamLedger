...
function build7CScript() {
  return `
let kelp7cNodes=[],kelp7cSocket=null,kelp7cJoined=false,kelp7cReplicationReady=false,kelp7cSubscriptionId=null,kelp7cRealtimeUpdates=0,kelp7cInventory={kelp:0,gold:0,pearls:0};
...
async function kelp7cLoadInventory(){
  ...
  const raw=await kelp7bRpc('kelplantis_get_player',{p_token:state.id});
  const row=Array.isArray(raw)?raw[0]:raw;const inv=(row&&row.inventory)||{};
  kelp7cInventory={kelp:Number(inv.kelp)||0,gold:Number(inv.gold)||0,pearls:Number(inv.pearls)||0};update7cInventoryUi();return kelp7cInventory;
}
...
function kelp7cStartRealtime(){
  ...
  kelp7cSocket.onmessage=e=>{let m;try{m=JSON.parse(e.data);}catch(_){return;}if(m.event==='phx_reply'&&m.payload&&m.payload.status==='ok'){const subs=m.payload.response&&m.payload.response.postgres_changes;kelp7cSubscriptionId=subs&&subs[0]&&subs[0].id;kelp7cJoined=true;return;}if(m.event==='system'&&m.payload&&m.payload.extension==='postgres_changes'){kelp7cReplicationReady=m.payload.status==='ok';return;}if(m.event==='postgres_changes'){const ids=m.payload&&m.payload.ids||[];if(kelp7cSubscriptionId!==null&&!ids.includes(kelp7cSubscriptionId))return;kelp7cRealtimeUpdates++;kelp7cLoadNodes();}};
  kelp7cSocket.onclose=()=>{kelp7cJoined=false;kelp7cReplicationReady=false;kelp7cSubscriptionId=null;};
}
window.__KELPLANTIS_7C__={loadNodes:kelp7cLoadNodes,harvest:kelp7cHarvestNode,getNodes:()=>kelp7cNodes,getInventory:()=>({...kelp7cInventory}),getPlayer:kelp7cPlayer,realtimeUpdates:()=>kelp7cRealtimeUpdates,realtimeJoined:()=>kelp7cJoined,realtimeReplicationReady:()=>kelp7cReplicationReady};
