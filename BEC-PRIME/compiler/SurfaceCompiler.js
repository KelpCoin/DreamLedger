...
const TEMPLATE = path.join(ROOT, 'surface', 'index.template.html');
...
function assertClean(label, content) {
  const lower = String(content).toLowerCase();
  const hit = FORBIDDEN_PUBLIC.find(token => lower.includes(token.toLowerCase()));
  if (hit) throw new Error(`PUBLIC_SURFACE_GATE_FAILED: ${label} contains forbidden token: ${hit}`);
}
...
if (marketplaceRuntime) assertClean('marketplace runtime', marketplaceRuntime);

// The template is intentionally free of catalog values. Runtime APIs provide live inventory,
// prices and auction state so the browser cannot manufacture commerce truth.
write(INDEX, template);
assertClean('compiled index', fs.readFileSync(INDEX, 'utf8'));
...
const build = {
  type: 'dreamledger-public-surface-compilation',
  status: 'PASS',
  compiler: 'surface',
  ...
  source_hashes: {
    ...
    marketplace_runtime: fs.existsSync(ASSET) ? digest(ASSET) : null
  },
  ...
  gates: {
    ...
    forbidden_public_tokens_checked: true,
