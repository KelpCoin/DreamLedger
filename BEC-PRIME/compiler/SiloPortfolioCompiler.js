...
const sourceHash = digest(fs.readFileSync(SOURCE, 'utf8'));
const catalog = {
...
function page(wedge) {
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow"><title>${esc(wedge.name)} | DreamLedger</title>
<style>:root{--bg:#090a0d;--panel:#151820;--line:#303541;--text:#f4f1eb;--muted:#a8adb8;--gold:#d8b66b;--pink:#e5a6c4}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 70% -10%,#241a26,#090a0d 48%);color:var(--text);font:16px/1.55 system-ui,-apple-system,Segoe UI,sans-serif}.wrap{max-width:900px;margin:auto;padding:28px 20px 60px}.eyebrow{color:var(--pink);font-size:.72rem;font-weight:900;letter-spacing:.16em;text-transform:uppercase}h1{font-size:clamp(2.5rem,7vw,5.4rem);line-height:.92;letter-spacing:-.055em;margin:12px 0 18px}p{color:var(--muted);max-width:720px}.panel{border:1px solid var(--line);background:rgba(21,24,32,.88);border-radius:18px;padding:22px;margin:18px 0}.label{font-size:.7rem;color:var(--gold);font-weight:900;letter-spacing:.12em;text-transform:uppercase}.cta{display:inline-block;margin-top:10px;padding:13px 18px;border-radius:11px;background:var(--gold);color:#111;text-decoration:none;font-weight:900}.back{color:var(--muted);text-decoration:none}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media(max-width:680px){.grid{grid-template-columns:1fr}}</style></head>
<body><main class="wrap"><a class="back" href="/portfolio/">&larr; Back to portfolio</a><div class="eyebrow">DreamLedger / compiled wedge ${String(wedge.rank).padStart(2,'0')}</div><h1>${esc(wedge.name)}</h1><p>${esc(wedge.wedge)}</p><div class="grid"><section class="panel"><div class="label">Buyer</div><p>${esc(wedge.buyer)}</p></section><section class="panel"><div class="label">Monetization</div><p>${esc(wedge.monetization)}</p></section></div><section class="panel"><div class="label">Compiled tunnel</div><p>CTA &rarr; proposal &rarr; human review &rarr; canonical offer &rarr; activation only after approval. No checkout is implied by this page.</p><a class="cta" href="mailto:hello@dreamledger.org?subject=${encodeURIComponent('DreamLedger: '+wedge.name)}">Request this wedge</a></section><section class="panel"><div class="label">Billboard SKU</div><p>This surface is a billboard SKU: it is an indexed entry point into the service tunnel, not a claim that the service has already been sold or activated.</p></section></main></body></html>`;
}
...
const proof = {
  schema: 'BEC-PRIME/SILO-PORTFOLIO-COMPILATION/v2',
  status: 'PASS',
  source_hash: sourceHash,
  wedge_count: compiled.length,
  generated: {
  ...
  guarantees: { approval_required: true, checkout_available: false, payment_claimed: false, private_ip_exposed: false, silo_isolation_required: true, external_actions_allowed: false }
};
