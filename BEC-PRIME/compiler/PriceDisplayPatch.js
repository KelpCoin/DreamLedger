...
if (!fs.existsSync(ASSET)) {
  throw new Error('Price display patch input missing: compiled/website/assets/marketplace-live.js');
}
...
if (!source.includes(before)) {
  throw new Error('Expected marketplace money formatter was not found; refusing unsafe rewrite.');
}

source = source.replace(before, after);
fs.writeFileSync(ASSET, source, 'utf8');
console.log('Price display patch applied: integer product prices >= 1000 are rendered as cents/dollars while existing dollar-denominated catalog prices remain unchanged.');
