...
async function handle(req, res, url) {
  ...
  if (req.method === 'POST' && route === '/api/account/register') {
    ...
    const user = { id, name, email, email_verified: false, avatar: null, avatar_style: null, dreamiez_linked: false, cosmetics: [], streak: 0, lastVisit: null, guild: null, seller: { display_name: name, location: '', bio: '' }, password: passwordRecord(password), account_created_at: new Date().toISOString() };
  ...
  if (req.method === 'POST' && route === '/api/account/update') {
    ...
    const user = users.find(item => item.id === sessionId && item.email); if (!user) return json(res, 401, { error: 'Please log in first.' });
    const input = await body(req); if (input.name !== undefined) user.name = String(input.name || '').trim().slice(0, 60) || user.name;
    user.seller = { display_name: String(input.seller_display_name || user.seller?.display_name || user.name).trim().slice(0, 80), location: String(input.location || '').trim().slice(0, 100), bio: String(input.bio || '').trim().slice(0, 500) };
    writeUsers(users); return json(res, 200, { ok: true, account: publicAccount(user) });
  }
  return false;
}
