...
const handler = require('./[...route].js');
...
(async () => {
  ...
  console.log(JSON.stringify({ status: 'PASS', register: registered.status, me: me.status, logout: loggedOut.status, login: login.status, dependency_root: 'compiled/website/api/account' }));
