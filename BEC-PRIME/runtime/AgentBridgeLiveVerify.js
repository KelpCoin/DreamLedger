...
async function run() {
  ...
  if (!configured) {
    return {
      ...
      checks: {
        ...
        dreamledger_agent_bridge_token: Boolean(process.env.DREAMLEDGER_AGENT_BRIDGE_TOKEN)
      },
  ...
  const headers = { 'x-dreamledger-agent-token': process.env.DREAMLEDGER_AGENT_BRIDGE_TOKEN };
  ...
  try {
    ...
    if (manifest.json?.schema_version !== 'BECK-AGENT-BRIDGE-1.1') throw new Error('manifest schema mismatch');
    ...
    return {
      ...
      configured: true,
      manifest: 'PASS',
      authenticated_state_read: 'PASS',
      authenticated_jobs_read: 'PASS',
      normalized_contract: 'PASS',
      ra000001_status: state.json.state.status,
      verified_payment_count: Number(state.json.state.verified_payment_count || 0),
      revenue_nzd: Number(state.json.state.revenue_nzd || 0),
