...
async function claimTask() {
  ...
  const roles = ['hunter', 'adversary', 'synthesizer', 'verifier'];
  for (const role of roles) {
    const rows = await rpc('claim_economic_model_task', {
      p_model_name: model,
      p_model_role: role,
...
async function completeTask(task, assessmentId) {
  const path = `economic_model_tasks?task_id=eq.${encodeURIComponent(task.task_id)}`;
...
async function main() {
  ...
  try {
    ...
    const assessmentId = await rpc('record_economic_assessment', {
      p_candidate_id: task.candidate_id,
      p_model_name: task.model_name,
      p_model_role: task.model_role,
      p_overall_score: number01(a.overall_score, 'overall_score'),
      p_evidence_tier: String(a.evidence_tier || 'UNVERIFIED').slice(0, 64),
      p_freshness_days: Math.max(0, Math.floor(Number(a.freshness_days || 0))),
      p_buyer_intent: Boolean(a.buyer_intent),
      p_seller_side: Boolean(a.seller_side),
      p_primary_source_verified: Boolean(a.primary_source_verified),
      p_scope_fit: Boolean(a.scope_fit),
      p_payment_path: Boolean(a.payment_path),
      p_confidence: number01(a.confidence, 'confidence'),
      p_reasoning: String(a.reasoning || '').slice(0, 12000),
      p_raw_assessment: a,
      p_source_url: a.source_url == null ? null : String(a.source_url).slice(0, 2000)
    });
    ...
    return { status: 'SUCCEEDED', task_id: task.task_id, assessment_id: id };
  } catch (error) {
