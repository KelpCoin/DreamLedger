...
async function handle(req, res) {
  ...
  try {
    ...
    if (existing && existing.receipt && existing.receipt.status === 'SUCCEEDED') {
      return send(res, 200, Object.assign({}, existing.receipt, { idempotent: true }), correlation);
    }
    ...
    if (!ok) {
      const raced = await findReceipt(eventId);
      if (raced && raced.receipt && raced.receipt.status === 'SUCCEEDED') {
        return send(res, 200, Object.assign({}, raced.receipt, { idempotent: true }), correlation);
      }
      return send(res, 409, { [op === 'complete' ? 'completed' : 'failed']: false, reason: 'LEASE_NOT_HELD', correlation_id: correlation, fenced: true }, correlation);
    }
    ...
    const saved = await remember(eventId, receipt);
    if (saved.idempotent && saved.receipt) return send(res, 200, Object.assign({}, saved.receipt, { idempotent: true }), correlation);
