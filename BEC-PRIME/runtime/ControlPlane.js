...
const executor = require('./EconomicExecutorV1');
...
function boot() {
  ...
  let manifestResult; try { manifestResult = mcpSecurity.verifyToolManifest(); } catch (err) { manifestResult = { status: 'FAIL', error: err.message }; }
  lastBoot = { compile: compileResults, security_gauntlet: securityResult, legacy_gauntlet: legacyResult, sentinel: sentinelResult, ledger: ledgerResult, fossils: fossilResult, mcp_manifest: manifestResult, truth_oracle: truthOracle.snapshot(), workers: scheduler.advertisedWorkers().workers, worker_pool: { status: 'READY', queue: workerPool.listJobs().length }, status: compileResults.every(x => x.status === 'PASS') && securityResult.status === 'PASS' && sentinelResult.verdict === 'PASS' && ledgerResult.status === 'PASS' && fossilResult.status === 'PASS' && manifestResult.status === 'PASS' ? 'PASS' : 'FAIL', checked_at: new Date().toISOString() };
  return lastBoot;
}
function health() {
  return { control_plane: 'ELOHIM-V7', gauntlet: 'SECURITY-GAUNTLET-V7', compiler_mirror: 'COMPILER-MIRROR-V1', mcp_gateway: 'DREAMLEDGER-GATEWAY-V2', economic_court: 'ECONOMIC-COURT-V2', executor: 'APPROVAL-TOKEN-GATED', digital_proxy: 'approval-gated', scheduler: { registry: scheduler.loadRegistry(), workers: scheduler.advertisedWorkers().workers }, ledger: ledger.verifyChain(), fossils: fossil.verifyFossils(), truth_oracle: truthOracle.snapshot(), mcp_manifest: (() => { try { return mcpSecurity.verifyToolManifest(); } catch (e) { return { status: 'FAIL', error: e.message }; } })(), worker_pool: { status: 'READY', jobs: workerPool.listJobs() }, boot: lastBoot || { status: 'NOT_BOOTED' } };
}
...
function controlMutationAuthorized(req) { const configured = process.env.DREAMLEDGER_CONTROL_TOKEN || process.env.DREAMLEDGER_INTERNAL_TRUST_TOKEN || ''; const supplied = String(req.headers['x-dreamledger-control-token'] || req.headers['x-dreamledger-internal-token'] || ''); return Boolean(configured) && cryptoSafeEqual(supplied, configured); }
function cryptoSafeEqual(a, b) { const A = Buffer.from(String(a)); const B = Buffer.from(String(b)); return A.length === B.length && require('crypto').timingSafeEqual(A, B); }
...
async function handle(req, res) {
  ...
  if (req.method === 'POST' && url === '/api/control/economic/approve') { if (denyMutation(req, res)) return true; try { const input = await readJsonBody(req); const approved = court.approve(input.proposal, req.headers['x-human-approver'] || 'HUMAN'); return send(200, approved); } catch (err) { return send(403, { error: err.message }); } }
  if (req.method === 'POST' && url === '/api/control/economic/execute') { if (denyMutation(req, res)) return true; try { const input = await readJsonBody(req); return send(200, await executor.executeApprovedCheckout(input.proposal, input.approval_token)); } catch (err) { return send(403, { error: err.message }); } }
