...
function boot() {
  ...
  lastBoot = {
    ...
    status: compileResults.every(x => x.status === 'PASS') && gauntletResult.status === 'PASS' && sentinelResult.verdict === 'PASS' ? 'PASS' : 'FAIL',
...
function health() {
  return {
    control_plane: 'ELOHIM-V6',
    gauntlet: 'GAUNTLET-V6',
...
async function handle(req, res) {
  ...
  if (req.method === 'POST' && url === '/api/control/demand/record') {
    try { const input = await readJsonBody(req); return send(201, demandRadar.record(input.type || 'manual_signal', input)); }
  ...
  if (req.method === 'POST' && url === '/api/control/jobs') {
    try {
      const input = await readJsonBody(req);
      const job = workerPool.createJob(input);
      return send(201, { status: 'QUEUED', job });
    } catch (err) { return send(400, { error: err.message }); }
  }
  if (req.method === 'POST' && url === '/api/control/jobs/run-once') {
    const result = await workerPool.runNext();
    return send(result.status === 'FAILED' ? 500 : 200, result);
  }
  if (req.method === 'GET' && url.startsWith('/api/control/jobs/')) {
    const jobId = url.slice('/api/control/jobs/'.length);
    try { return send(200, workerPool.loadJob(jobId)); } catch (err) { return send(404, { error: err.message }); }
  }

  if (req.method === 'POST' && url === '/api/control/elohim/propose') {
    try { return send(200, await elohim.propose(await readJsonBody(req))); }
    catch (err) { return send(400, { error: err.message }); }
  }
  if (req.method === 'POST' && url === '/api/control/proxy/queue') {
    try { const input = await readJsonBody(req); return send(201, proxy.queue(input.action, input.payload, input.requested_by)); }
    catch (err) { return send(400, { error: err.message }); }
  }
  if (req.method === 'POST' && url.startsWith('/api/control/proxy/approve/')) {
    const id = url.slice('/api/control/proxy/approve/'.length);
    try { return send(200, proxy.approve(id, req.headers['x-human-approver'] || 'human', req.headers['x-digital-proxy-token'] || '')); }
    catch (err) { return send(403, { error: err.message }); }
  }
  return false;
}
