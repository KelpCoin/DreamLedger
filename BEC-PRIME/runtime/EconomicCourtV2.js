...
function propose(input) {
  ...
  const proposal = {
    proposal_id: uuid(action === 'checkout' ? 'CHK' : 'OFF'),
    schema_version: 'ECONOMIC-COURT-2.0',
    state: 'PROPOSED',
    action,
    silo: validateSilo(input.silo),
    created_at: new Date().toISOString(),
    approval_required: true,
    autonomous_spend_limit_minor: ZERO,
    payload: input.payload || {}
  };
  if (action === 'checkout') {
    proposal.sku = String(input.payload?.sku || '');
    proposal.amount_minor = amountMinor(input.payload?.amount);
    proposal.currency = String(input.payload?.currency || 'NZD').toUpperCase();
    proposal.customer_ref = validateCustomerRef(input.payload?.customer_ref);
  }
...
function approvalPayload(proposal, approver, expiresAt, nonce) {
  return { approval_id: uuid('APP'), checkout_id: proposal.proposal_id, proposal_hash: hash({ proposal_id: proposal.proposal_id, sku: proposal.sku, amount_minor: proposal.amount_minor, currency: proposal.currency, silo: proposal.silo }), approved_by: approver || 'HUMAN', approved_at: new Date().toISOString(), expires_at: expiresAt, approval_nonce: nonce, allowed_sku: proposal.sku || null, allowed_amount_minor: proposal.amount_minor || null, allowed_currency: proposal.currency || null, allowed_silo: proposal.silo, version: 'APPROVAL-2.0' };
}
...
function verifyApproval(proposal, token) {
  ...
  verifier.update(stable(unsigned));
  verifier.end();
  const valid = verifier.verify(publicKey, signature, 'base64url');
  return valid ? { valid: true } : { valid: false, reason: 'signature invalid' };
}
function executable(proposal, token) {
  const verification = verifyApproval(proposal, token);
  if (!verification.valid) return { executable: false, state: 'BLOCKED', reason: verification.reason };
  return { executable: true, state: 'EXECUTABLE', approval_id: token.approval_id };
}
