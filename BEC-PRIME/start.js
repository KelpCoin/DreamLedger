...
if(req.method==='POST'&&requestPath==='/api/offer-checkout/create'){try{const requestBody=await jsonBody(req);const productOffer=approvedProductOffer(requestBody.offer_id);if(productOffer)return directProductCheckout(res,productOffer.offer_id,productOffer.silo);const verified=loadVerifiedOffers().find(item=>item.offer_id===requestBody.offer_id);if(!verified)return send(res,403,{error:'Offer is not approved for checkout'});return originalHandler(replayRequest(req,JSON.stringify(requestBody)),res)}catch(err){return send(res,400,{error:err.message||'Invalid checkout request'})}}
