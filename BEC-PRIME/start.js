...
http.createServer = function wrappedCreateServer(...args) {
  ...
  args[0] = async function dreamledgerRuntimeHandler(req, res) {
    ...
    if (req.method === 'POST' && requestPath === '/webhook') {
      try {
        const platformResult = await platformCart.handleWebhook(req, res);
        if (platformResult.handled) return;
        const omniResult = await omniCommerce.handleWebhook(replayRequest(req, platformResult.raw), res);
        if (omniResult.handled) return;
        return originalHandler(replayRequest(req, platformResult.raw), res);
      } catch (err) { return send(res, 400, { error: err.message || 'Webhook rejected' }); }
    }

    try {
      if (await platformCart.handle(req, res, requestPath)) return;
    } catch (err) { return send(res, 500, { error: err.message || 'Platform cart route failed' }); }
    try {
      if (await omniCommerce.handle(req, res, requestPath)) return;
    } catch (err) { return send(res, 500, { error: err.message || 'Omni-commerce route failed' }); }
