...
function staticFile(req,res){ const route=String(req.url||'').split('?')[0]; if(!route.startsWith('/'))return false; const aliases={'/':'index.html','/login':'login.html','/login/':'login.html','/register':'register.html','/register/':'register.html','/account':'account.html','/account/':'account.html','/chat':'chat.html','/chat/':'chat.html'}; let rel=aliases[route]||route.slice(1); if(!rel||rel.includes('..'))return send(res,400,{error:'invalid path'}),true; const file=path.normalize(path.join(PUBLIC_ROOT,rel)); if(!file.startsWith(PUBLIC_ROOT+path.sep)&&file!==PUBLIC_ROOT)return send(res,403,{error:'Forbidden'}),true; fs.readFile(file,(err,data)=>{ if(err)return send(res,404,{error:'Not found'}); const ext=path.extname(file).toLowerCase(); res.writeHead(200,{'Content-Type':MIME[ext]||'application/octet-stream','Cache-Control':'no-store'}); res.end(data); }); return true; }

http.createServer=function wrappedCreateServer(...args){
  const originalHandler = typeof args[0]==='function' ? args[0] : staticFile;
  args[0]=async function dreamledgerRuntimeHandler(req,res){
    ...
    try{if(await conversations.handle(req,res,requestPath))return;}catch(err){return send(res,500,{error:err.message||'Conversation route failed'});}
    if(req.method==='POST'&&requestPath==='/webhook'){
      const marketplaceResult=await marketplaceWebhook.handle(req,res); if(marketplaceResult.handled)return;
      const raw=marketplaceResult.raw||'';
      try{const platformResult=await platformCart.handleWebhook(replayRequest(req,raw),res);if(platformResult.handled)return;const omniResult=await omniCommerce.handleWebhook(replayRequest(req,raw),res);if(omniResult.handled)return;return originalHandler(replayRequest(req,raw),res);}catch(err){return send(res,400,{error:err.message||'Webhook rejected'});}
    }
    ...
    if(req.method==='GET'&&requestPath.startsWith('/dreamiez/')){const rel=requestPath.slice('/dreamiez/'.length)||'dreamiez.html';const root=path.join(__dirname,'silos','SILO_DREAMIEZ','compiled','website');const file=path.normalize(path.join(root,rel));if(!file.startsWith(root+path.sep))return send(res,403,{error:'Forbidden'});return fs.readFile(file,(err,data)=>{if(err)return send(res,404,{error:'Dreamiez page not found'});const ext=path.extname(file).toLowerCase();res.writeHead(200,{'Content-Type':MIME[ext]||'application/octet-stream','Cache-Control':'no-store'});res.end(data);});}
...
capturedServer=originalCreateServer((req,res)=>staticFile(req,res));
