...
function publicAuction(a, bids) {
  ...
  return {
    ...
    current_price: top?.amount || a.start_price,
    reserve_price: a.reserve_price,
    reserve_met: reserveMet,
    minimum_increment: a.minimum_increment,
    buy_now_price: a.buy_now_price || null,
...
function createAuction(input) {
  ...
  const auction = {
    ...
    silo: String(input.silo),
    title: String(input.title),
    description: String(input.description || ''),
    currency: String(input.currency || 'NZD').toUpperCase(),
    start_price: Number(input.start_price),
    reserve_price: Number(input.reserve_price || 0),
    minimum_increment: Number(input.minimum_increment || 1),
    buy_now_price: input.buy_now_price == null ? null : Number(input.buy_now_price),
    starts_at: Number(input.starts_at || now()),
    ends_at: Number(input.ends_at),
    anti_sniping_seconds: Number(input.anti_sniping_seconds || 120),
    status: 'scheduled',
    approval_required: input.approval_required !== false,
    checkout_available: input.checkout_available === true
  };
...
function placeBid(auctionId, bidderId, amount) {
  ...
  const view = publicAuction(auction, state.bids);
  const t = now();
...
function listAuctions(silo) {
  const state = loadState();
  return state.auctions.filter(a => !silo || a.silo === silo).map(a => publicAuction(a, state.bids));
}
function getAuction(id) {
  const state = loadState();
  const auction = state.auctions.find(a => a.auction_id === id);
  return auction ? publicAuction(auction, state.bids) : null;
}
