...
function requestJson(url, options = {}, body = null) {
  return new Promise((resolve, reject) => {
    const req = https.request(url, {
      ...
      res.on('data', chunk => { data += chunk; if (data.length > 4000000) req.destroy(new Error('Response too large')); });
      res.on('end', () => {
...
function normaliseCandidate(item) {
  return {
    ...
    shipping: item.shippingOptions && item.shippingOptions[0] && item.shippingOptions[0].shippingCost ? Number(item.shippingOptions[0].shippingCost.value) : null,
...
async function searchEbay(wanted, options = {}) {
  ...
  if (!applicationToken && process.env.EBAY_APP_ID && process.env.EBAY_CERT_ID) applicationToken = await getEbayApplicationToken(process.env.EBAY_APP_ID, process.env.EBAY_CERT_ID);
  ...
  params.set('filter', buildEbayFilter(wanted, process.env.EBAY_PRICE_CURRENCY || 'AUD'));
  ...
  return { platform: 'ebay', status: 'OK', marketplace, query: params.get('q'), filter: params.get('filter'), candidates: (data.itemSummaries || []).map(normaliseCandidate), total: Number(data.total || 0) };
}
function containsAny(haystack, needles) {
  const text = String(haystack || '').toLowerCase();
  return needles.length > 0 && needles.some(n => text.includes(String(n).toLowerCase()));
}
...
function rankCandidate(candidate, wanted) {
  ...
  const parts = [['brand', wanted.brand, 0.30], ['size', wanted.size, 0.20], ['colour', wanted.colour, 0.15], ['era', wanted.era, 0.15], ['style', wanted.style, 0.05]];
  const scores = {};
  let numerator = 0;
  let denominator = 0;
  for (const [name, value, weight] of parts) {
    const result = scoreField(title, value, weight);
    if (result.weight) { scores[name] = result.score; numerator += result.score * result.weight; denominator += result.weight; }
  }
  const p = priceScore(candidate, wanted);
  if (p.weight) { scores.price = p.score; numerator += p.score * p.weight; denominator += p.weight; }
...
function loadWanted(wantedId) {
  ...
  return readJsonl(file).find(item => item.id === wantedId) || null;
}
async function hunt(wanted, options = {}) {
  ...
  const ranked = rankCandidates(rawCandidates, wanted).map(candidate => ({ ...candidate, wanted_id: wantedId, discovered_at: new Date().toISOString() }));
  for (const candidate of ranked) appendJsonl(CANDIDATE_FILE, candidate);
  const session = { id: 'H-' + Date.now().toString(36), wanted_id: wantedId, started_at: started, completed_at: new Date().toISOString(), candidate_count: ranked.length, best_match_score: ranked.length ? ranked[0].match_score : 0, adapters };
