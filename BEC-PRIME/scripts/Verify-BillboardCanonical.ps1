...
try{$x=Get-Text "$BaseUrl/billboard";Check 'GLOBAL_ROUTE' ($x -match 'NZ\$50' -and $x -match 'Founding Tile' -and $x -match 'Swipe') 'canonical billboard surface'}catch{Check 'GLOBAL_ROUTE' $false $_.Exception.Message}
...
try{$r=Invoke-WebRequest -Uri "$BaseUrl/api/billboard/inventory/NZ" -UseBasicParsing -TimeoutSec 20;$j=$r.Content|ConvertFrom-Json;Check 'NZ_INVENTORY_API' ($r.StatusCode -eq 200 -and $j.market -eq 'NZ' -and $j.founding_positions -eq 100 -and $null -ne $j.founding_positions_remaining) 'market-scoped finite inventory API'}catch{Check 'NZ_INVENTORY_API' $false $_.Exception.Message}
try{$r=Invoke-WebRequest -Uri "$BaseUrl/api/billboard" -UseBasicParsing -TimeoutSec 20;$j=$r.Content|ConvertFrom-Json;Check 'BILLBOARD_CONTROL_API' ($r.StatusCode -eq 200 -and $j.canonical -eq '/billboard' -and $j.scarcity -match '100') 'canonical billboard control surface'}catch{Check 'BILLBOARD_CONTROL_API' $false $_.Exception.Message}
