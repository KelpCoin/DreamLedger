param([Parameter(Mandatory=$true)][string]$Url,[string[]]$CompareIds=@(),[switch]$PublishApproved)
$ErrorActionPreference='Stop'
$repo=Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
$node=Join-Path $repo 'BEC-PRIME\edh\EDHOneLinkPipeline.js'
$args=@($node,"--url=$Url")
if($CompareIds.Count -gt 0){$args += "--compare=$($CompareIds -join ',')"}
if($PublishApproved){$args += '--publish-approved'}
& node @args
