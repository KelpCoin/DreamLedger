...
function metadataPresent(windowText, key) {
  ...
  const paymentIntent = new RegExp('payment_intent_data[^\\n]{0,220}metadata[^\\n]{0,220}(?:\\[' + escaped + '\\]|["\\\']' + escaped + '["\\\'])', 'i');
  const subscription = new RegExp('subscription_data[^\\n]{0,220}metadata[^\\n]{0,220}(?:\\[' + escaped + '\\]|["\\\']' + escaped + '["\\\'])', 'i');
  return paymentIntent.test(windowText) || subscription.test(windowText);
}
...
const dedup = new Map(); for (const p of producers) dedup.set(p.file + ':' + p.lines.join('-'), p);
const rows = [...dedup.values()]; const failures = rows.filter((r) => r.missing.length);
const proofDir = path.join(ROOT, 'data', 'proofs'); fs.mkdirSync(proofDir, { recursive: true });
const proof = { schema: 'DREAMLEDGER.STRIPE_CHECKOUT_CONTRACT.v3', generated_utc: new Date().toISOString(), required_payment_intent_metadata: REQUIRED, producer_windows: rows, producer_window_count: rows.length, failing_window_count: failures.length, status: failures.length ? 'FAIL' : 'PASS' };
