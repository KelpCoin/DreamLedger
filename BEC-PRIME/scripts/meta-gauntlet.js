...
const ledger = read('lib/revenueLedger.js');
const render = read('render.yaml');
const surface = read('compiled/website/index.html');
const publicRuntime = read('compiled/website/assets/public-marketplace.js');
...
check('NO_PUBLIC_SECRET_MARKERS', !/sk_(live|test)_[A-Za-z0-9]+|whsec_[A-Za-z0-9]+|-----BEGIN (?:RSA |OPENSSH )?PRIVATE KEY-----/.test(read('catalog/ip-capabilities.json')), 'secret marker in internal capability catalog');
check('PRIVATE_IP_NOT_PUBLIC', surfaceManifest && surfaceManifest.surface_policy.internal_capability_catalog_public === false && surfaceManifest.private_surfaces?.some(x => x.route === '/api/ip' && x.status === 'removed_from_public_runtime') && !surface.includes('/api/ip') && !publicRuntime.includes('/api/ip'), 'private capability catalog remains exposed by the public surface');
check('PUBLIC_SURFACE_SANITIZED', !surface.includes('BEC-PRIME IP / Commercial Surfaces') && !surface.includes('capability catalog') && surface.includes('private implementation material is not a public surface'), 'public surface still contains private implementation material');
check('PUBLIC_RUNTIME_SANITIZED', !publicRuntime.includes('/api/ip') && !publicRuntime.includes('injectCapabilityControl'), 'public runtime still contains IP catalog exposure');
...
check('PUBLIC_SURFACE_ENGINE_BACKED', surface.includes('/api/products') || publicRuntime.includes('/api/products'), 'public surface is not clearly engine-backed');
...
check('REVENUE_LEDGER_PRESENT', ledger.includes('STRIPE_CLEARING') && ledger.includes('DEFERRED_REVENUE') && ledger.includes('SALES_REVENUE') && ledger.includes('Unbalanced journal entry'), 'double-entry revenue ledger missing');
...
check('STRIPE_WEBHOOK_PROOF', server.includes('checkout.session.completed') && server.includes('event.id') && server.includes('FIRST_PAYMENT_PROOF'), 'payment proof boundary missing');
check('FULFILLMENT_FOSSIL', server.includes('READY_FOR_DELIVERY') && server.includes('createFulfillment'), 'fulfillment fossil missing');
