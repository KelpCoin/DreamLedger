...
check('ELOHIM_APPROVAL_BOUNDARY', elohim.includes('approval') || elohim.includes('PROPOSED'), 'Elohim proposal boundary missing');
...
check('DREAMIEZ_E2E_SMOKE', smoke.status === 0, (smoke.stdout || smoke.stderr || '').trim().slice(-1200));

const result = {
  schema: 'BEC-PRIME/META-GAUNTLET/v1',
  verified_at: new Date().toISOString(),
  red_team: {
    focus: ['silo leakage', 'unlocked offers', 'public secret markers', 'fake live integrations', 'payment proof spoofing', 'auction boundary']
  },
  blue_team: {
    focus: ['canonical catalogs', 'server-authoritative checkout', 'Elohim/Gauntlet control plane', 'Digital Proxy approval', 'Render health gate', 'Dreamiez persistence smoke']
  },
  checks,
  failures,
