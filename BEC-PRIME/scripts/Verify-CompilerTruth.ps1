...
if ([string]::IsNullOrWhiteSpace($ProofPath)) {
    $ProofPath = if ($env:BEC_COMPILER_TRUTH_PROOF) { $env:BEC_COMPILER_TRUTH_PROOF } else { Join-Path $ProofDir 'COMPILER-TRUTH-PROOF.json' }
}
...
foreach ($item in @(
    @($ContractPath,'contract'), @($ManifestPath,'cube_manifest'), @($OffersPath,'offers'),
    @($ApprovedPath,'approved_offers'), @($IpPath,'ip_catalog'), @($SurfaceCompilerPath,'surface_compiler'),
...
if ($offers) {
    ...
    foreach ($approvedId in $approvedIds) {
        if ($approvedId -eq 'OFFER-CMD-DIAG-29-NZD') { continue }
        if ($approvedId -notin @($offers.offers | ForEach-Object { [string]$_.offer_id })) {
            $Errors.Add("APPROVED_OFFER_MISSING:$approvedId")
        }
    }
}

if ($approved) {
    $cmd = @($approved.approved | Where-Object { $_.offer_id -eq 'OFFER-CMD-DIAG-29-NZD' }) | Select-Object -First 1
    if (-not $cmd) {
        $Errors.Add('CANONICAL_COMMANDER_OFFER_MISSING')
    } else {
        if ($cmd.price -ne 29) { $Errors.Add('CANONICAL_COMMANDER_PRICE') }
        if ($cmd.product_id -ne 'COMMANDER-DECK-DIAGNOSTIC-001') { $Errors.Add('CANONICAL_COMMANDER_PRODUCT') }
    }
}
...
$proof = [ordered]@{
    ...
    excluded_surfaces = @{ cinema='not part of production compile'; dreamiez='not part of production compile' }
    counts = [ordered]@{
...
if ($proof.verdict -eq 'PASS') { exit 0 } else { exit 1 }
