...
foreach($item in @(
 ...
 @($DreamiezTemplate,'dreamiez_template'),@($DreamiezJsTemplate,'dreamiez_client_template'),@($CompiledIndex,'compiled_index'),
 @($CompiledAsset,'compiled_marketplace_asset'),@($CompiledDreamiez,'compiled_dreamiez'),@($CompiledDreamiezJs,'compiled_dreamiez_client'),
...
$proof=[ordered]@{schema='BEC-PRIME/COMPILER-TRUTH-PROOF/v3';checked_at_utc=(Get-Date).ToUniversalTime().ToString('o');verdict=if($Errors.Count -eq 0){'PASS'}else{'FAIL'};compiler_chain=@('OfferCompiler','SurfaceCompiler','PriceDisplayPatch','Gauntlet','Meta-Gauntlet','Sentinel');surface_generation=[ordered]@{template_owned=($Errors -notcontains 'SURFACE_COMPILER_NOT_GENERATIVE');deterministic=($Errors -notcontains 'SURFACE_COMPILER_NONDETERMINISTIC_TIMESTAMP');output_matches_templates=($Errors -notlike 'SURFACE_OUTPUT_DRIFT:*');catalog_ids_not_embedded=($Errors -notlike 'SURFACE_HARDCODED_OFFER_ID:*');dreamiez_compiled=($Errors -notlike 'DREAMIEZ_*')};revenue_boundary=[ordered]@{all_offers_locked=($Errors -notlike 'OFFER_UNLOCKED:*' -and $Errors -notlike 'OFFER_CHECKOUT_ENABLED:*');approval_governed=($contract.surface_rules.approval_required_before_activation -eq $true)};counts=[ordered]@{capabilities=@($ip.capabilities).Count;offers=@($offers.offers).Count;public_surfaces=@($manifest.public_surfaces).Count};errors=@($Errors);doctrine='Canonical catalogs define commercial truth. Public surfaces are compiler outputs. Dreamiez account creation is a user journey, not an economic bypass.'}
