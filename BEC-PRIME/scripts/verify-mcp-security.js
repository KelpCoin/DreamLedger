...
function check(id, fn) {
  ...
  catch (err) { checks.push({ id, status: 'FAIL', error: err.message }); }
}
...
check('CONTROL_MUTATIONS_AUTHENTICATED', () => {
  assert.ok(controlPlane.includes('DREAMLEDGER_CONTROL_TOKEN'));
  assert.ok(controlPlane.includes('rejectMutation(req, send)'));
});
const start = fs.readFileSync(path.join(ROOT, 'start.js'), 'utf8');
check('WEB_SECURITY_HEADERS', () => {
  for (const h of ['X-Content-Type-Options','X-Frame-Options','Referrer-Policy','Permissions-Policy']) assert.ok(start.includes(h));
});
