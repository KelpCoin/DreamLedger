...
function req(url, headers = {}) {
  return { method: 'GET', url, headers, socket: { remoteAddress: '198.51.100.7' } };
}

function signed(raw) {
  const timestamp = Math.floor(Date.now() / 1000);
  const digest = crypto.createHmac('sha256', process.env.STRIPE_WEBHOOK_SECRET).update(`${timestamp}.${raw}`, 'utf8').digest('hex');
...
function writeSyntheticBillboard() {
  ...
  fs.writeFileSync(file, JSON.stringify({
    ...
    ads: [{
      id: 'synthetic_ad_0001', market: 'NZ', sku: 'BILLBOARD-SMALL', status: 'AWAITING_PAYMENT',
      size: 'small', size_label: 'Founding Tile', w: 100, h: 100, x: 0, y: 0,
      title: 'Synthetic Commerce Test', name: 'Synthetic Buyer', email: 'synthetic@example.invalid',
      link: 'https://example.invalid/synthetic', mime: 'image/png', ext: '.png',
      image_filename: 'synthetic.png', created_at: new Date().toISOString(), automated_validation: 'PASS',
      payment_status: 'unpaid', payment_recorded: false, fulfillment_recorded: false
    }]
  }, null, 2) + '\n', 'utf8');
}
...
async function main() {
  ...
  const event = {
    ...
    data: { object: {
      id: 'cs_synthetic_0001', payment_status: 'paid', amount_total: 5000, currency: 'nzd',
      metadata: { product: 'DREAMLEDGER-BILLBOARD', ad_id: 'synthetic_ad_0001' }
    }}
  };
