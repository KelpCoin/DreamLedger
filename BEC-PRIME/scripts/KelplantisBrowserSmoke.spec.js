...
test('Kelplantis Depth 1 7C two-session authoritative gathering slice', async ({ browser }) => {
  ...
  await expect.poll(async () => pageA.evaluate(() => Boolean(window.__KELPLANTIS_TEST__)), { timeout: 15000 }).toBe(true);
  await expect.poll(async () => pageB.evaluate(() => Boolean(window.__KELPLANTIS_TEST__)), { timeout: 15000 }).toBe(true);
