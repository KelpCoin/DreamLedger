...
const required=['index.html','dreamiez.html','assets/marketplace-live.js','assets/dreamiez-account.js','.well-known/agent-commerce.json','.well-known/ucp'];
const forbidden=[
  /api\/ip/i,
  /api\/control/i,
  /catalog\//i,
...
if(!index.includes('marketplace-live.js'))errors.push('MISSING:marketplace runtime');
...
const proof={schema:'dreamledger/public-surface-proof/v3',verdict:errors.length?'FAIL':'PASS',required_files:required,scanned_files:files.map(x=>path.relative(site,x)).sort(),errors,public_boundary:'customer commerce only; secrets, internal controls, implementation doctrine and private operational material excluded'};
