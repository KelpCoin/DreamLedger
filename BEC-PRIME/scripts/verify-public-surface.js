...
const required=['index.html','login.html','register.html','account.html','dreamiez.html','assets/dreamiez-account.js','.well-known/agent-commerce.json','.well-known/ucp'];
const forbidden=[
...
for(const rel of authPages){const p=path.join(site,rel);if(!fs.existsSync(p))continue;const raw=fs.readFileSync(p,'utf8');if(!/\/api\/account\//i.test(raw))errors.push(`ACCOUNT_CONTRACT:${rel}:missing /api/account/`);if(/\/api\/dreamiez\/account\/(login|register)/i.test(raw))errors.push(`ACCOUNT_CONTRACT:${rel}:must not use Dreamiez authentication`)}
...
const proof={schema:'dreamledger/public-surface-proof/v7',verdict:errors.length?'FAIL':'PASS',required_files:required,scanned_files:files.map(x=>path.relative(site,x).replace(/\\/g,'/')).sort(),excluded_implementation_dirs:Array.from(privateImplementationDirs).sort(),binary_assets_skipped:true,errors,public_boundary:'customer commerce, primary account surfaces, and intentionally published verification information only; private implementation material is not a public surface'};
