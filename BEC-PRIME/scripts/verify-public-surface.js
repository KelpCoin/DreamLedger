...
const proof={schema:'dreamledger/public-surface-proof/v15',verdict:errors.length?'FAIL':'PASS',required_files:required,scanned_files:files.map(x=>path.relative(site,x).replace(/\\/g,'/')).sort(),excluded_implementation_dirs:Array.from(privateImplementationDirs).sort(),excluded_surfaces:Array.from(excludedPublicPaths).sort(),excluded_files:Array.from(excludedPublicFiles).sort(),binary_assets_skipped:true,errors,public_boundary:'E1 public catalogue doorway; the homepage exposes only canonical storefront doors and isolated implementation surfaces remain outside the public front door'};
