...
for(const p of files){
  ...
  for(const re of forbidden){if(re.test(raw))errors.push(`PUBLIC_LEAK:${path.relative(site,p)}:${re}`)}
}
...
const proof={schema:'dreamledger/public-surface-proof/v4',verdict:errors.length?'FAIL':'PASS',required_files:required,scanned_files:files.map(x=>path.relative(site,x)).sort(),binary_assets_skipped:true,errors,public_boundary:'customer commerce only; secrets, internal controls, implementation doctrine and private operational material excluded'};
