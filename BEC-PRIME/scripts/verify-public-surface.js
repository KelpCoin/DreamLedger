...
const files=[];function walk(dir){for(const name of fs.readdirSync(dir)){const p=path.join(dir,name);const s=fs.statSync(p);if(s.isDirectory())walk(p);else files.push(p)}}
...
const agent=fs.existsSync(path.join(site,'.well-known','agent-commerce.json'))?JSON.parse(fs.readFileSync(path.join(site,'.well-known','agent-commerce.json'),'utf8')):{};
...
const proof={schema:'dreamledger/public-surface-proof/v1',verdict:errors.length?'FAIL':'PASS',required_files:required,scanned_files:files.map(x=>path.relative(site,x)).sort(),errors,public_boundary:'customer outcomes and machine discovery only; internal implementation doctrine excluded'};
