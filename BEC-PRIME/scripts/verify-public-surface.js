...
const required=['index.html','login.html','register.html','account.html','security.html','.well-known/agent-commerce.json','.well-known/ucp','truth-oracle.html','truth-oracle.json','transparency-policy.json'];
const forbidden=[
...
const securityPath=path.join(site,'security.html');
if(fs.existsSync(securityPath)){const securityText=fs.readFileSync(securityPath,'utf8');for(const phrase of ['human approval','proposal-only','no network']){if(!securityText.toLowerCase().includes(phrase))errors.push(`SECURITY_SURFACE_MISSING:${phrase}`)}}
