...
try {
    ...
    $result.status = 'CHECKOUT_CREATED_PAYMENT_UNPROVEN'; $result.checkout_url = $checkoutUrl; $result.session_id = $checkout.session_id; $result.amount_minor = $checkout.amount_minor; $result.currency = $checkout.currency
} catch {
    $result.status = 'BLOCKED'; $result.blocker = $_.ErrorDetails.Message; if (-not $result.blocker) { $result.blocker = $_.Exception.Message }
} finally {
...
Write-Host "STATUS: $($result.status)"; Write-Host "PRODUCT: $ProductId"
...
if ($result.status -eq 'CHECKOUT_CREATED_PAYMENT_UNPROVEN') { Write-Host 'NEXT: a real buyer must complete the checkout. A generated session is not a sale.'; exit 0 }
exit 1
