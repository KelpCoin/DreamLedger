...
need('PROOF-MTG-CATALOG-COMPILATION.json','MTG_COMPILE_PROOF');
...
const compileProof=JSON.parse(fs.readFileSync(path.join(ROOT,'PROOF-MTG-CATALOG-COMPILATION.json'),'utf8'));
...
if (compileProof.price_unit !== 'major_nzd') failures.push('MTG_PRICE_UNIT_MARKER=FAIL');
...
const result={schema:'BEC-PRIME/MTG-DREAMMEEZ-VERIFICATION/v1',status:failures.length?'FAIL':'PASS',utc:new Date().toISOString(),mtg_catalog:true,cinema:true,dreammeez_item_contract:true,price_unit:compileProof.price_unit,failures};
