...
function appendItem(item) { ensureStore(); fs.appendFileSync(DATA_FILE, JSON.stringify(item) + '\n', 'utf8'); }
...
function parseWantedText(text) {
  ...
  return {
    ...
    colour: [...new Set(detectedColours)].join(', ') || null,
...
function jsonBody(req) {
  return new Promise((resolve, reject) => {
    let data = '';
    req.on('data', chunk => { data += chunk; if (data.length > 100000) req.destroy(new Error('Request too large')); });
...
async function handle(req, res, requestPath) {
  ...
  if (requestPath === '/api/wanted' && req.method === 'POST') {
    try {
      ...
      const item = { id: 'W-' + crypto.randomBytes(6).toString('hex'), created_at: new Date().toISOString(), source_type: body.source_type || 'plain_text', source_platform: body.source_platform || 'universal', ...parsed };
      appendItem(item);
