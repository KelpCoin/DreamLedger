...
function product(id) {
  const file = path.join(PRODUCT_DIR, String(id) + '.json');
  return fs.existsSync(file) ? readJson(file) : null;
}

function config(id) {
  const file = path.join(CONFIG_DIR, String(id) + '.json');
  return fs.existsSync(file) ? readJson(file) : null;
}
...
function jsonBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      raw += chunk;
      if (raw.length > 200000) req.destroy(new Error('Request too large'));
    });
    req.on('end', () => {
...
function indexById(items) {
  const map = new Map();
  for (const item of Array.isArray(items) ? items : []) map.set(String(item.id), item);
  return map;
}
...
function validateSelection(c, body) {
  ...
  if (!land) throw new Error('INVALID_LAND_PACKAGE');
  ...
  for (const selection of selectedFlex) {
    ...
    if (!option) throw new Error('INVALID_FLEX_OPTION');
    if (option.status !== 'AVAILABLE') throw new Error('FLEX_OPTION_NOT_AVAILABLE');
    flexSelections.push({ slot_id: slotId, option_id: optionId, name: option.name, price_adjustment: Number(option.price_adjustment || 0) });
  }
  ...
  for (const id of selectedUpgrades) {
    ...
    if (!item) throw new Error('INVALID_PREMIUM_UPGRADE');
    if (item.status !== 'AVAILABLE') throw new Error('PREMIUM_UPGRADE_NOT_AVAILABLE');
    upgrades.push({ id: item.id, name: item.name, price_adjustment: Number(item.price_adjustment || 0) });
  }

  return { land, flex: flexSelections, upgrades };
}
...
async function handle(req, res, url) {
  ...
  if (req.method === 'GET' && url === prefix + '/decks') {
    ...
    const decks = files.map(name => readJson(path.join(CONFIG_DIR, name))).filter(item => item.status === 'LIVE_CONFIGURABLE').map(item => ({
      ...
      bracket_target: item.bracket_target,
  ...
  const match = url.match(new RegExp('^' + prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '/decks/([^/]+)(?:/(price|order))?$'));
  ...
  if (req.method === 'GET' && !action) {
    return send(res, 200, {
      ...
      theme: c.theme,
      strategy: c.strategy,
      bracket_target: c.bracket_target,
      base_price_minor: c.base_price_minor,
      currency: c.currency,
      testing_status: c.testing_status,
      testing_note: c.testing_note,
      primer_status: c.primer_status,
  ...
  if (req.method === 'POST' && action === 'price') {
    try {
      ...
      const selection = validateSelection(c, body);
      const priced = price(c, selection);
      return send(res, 200, { deck_id: c.deck_id, currency: c.currency, selection, pricing: priced });
    } catch (err) {
      return send(res, 400, { error: err.message || 'INVALID_CONFIGURATION' });
    }
  }
  ...
  if (req.method === 'POST' && action === 'order') {
    try {
      ...
      const order = {
        schema_version: 'mtg-configured-order-v1',
        order_id: orderId,
        deck_id: c.deck_id,
        product_id: p.id,
        silo: 'mtg',
      ...
      const params = {
        mode: 'payment',
        success_url: PUBLIC_BASE + '/checkout/success?configuration_order_id=' + encodeURIComponent(orderId),
        cancel_url: PUBLIC_BASE + '/mtg/configurator.html?deck=' + encodeURIComponent(c.deck_id) + '&checkout_cancelled=1',
        'metadata[product_id]': p.id,
        'metadata[silo]': 'mtg',
        'metadata[configuration_order_id]': orderId,
      ...
      order.checkout_created_at = new Date().toISOString();
      fs.writeFileSync(path.join(ORDERS_DIR, orderId + '.json'), JSON.stringify(order, null, 2) + '\n', 'utf8');
      return send(res, 200, { ok: true, order_id: orderId, checkout_url: session.url, pricing: priced, currency: c.currency });
    } catch (err) {
      return send(res, 400, { error: err.message || 'CHECKOUT_CREATION_FAILED' });
    }
  }
