...
function publicAccount(user) {
  return {
    ...
    avatar_style: user.avatar_style || 'dream',
    avatar: user.avatar || { height: 2, build: 2, skin: 5 },
...
async function handle(req, res, url) {
  ...
  const authRoute = route === '/api/account/register' || route === '/api/account/login' || route === '/api/account/logout' || route === '/api/account/me' || route === '/api/dreamiez/account/create' || route === '/api/dreamiez/account/login' || route === '/api/dreamiez/account/logout' || route === '/api/dreamiez/me';
  ...
  if (req.method === 'POST' && (route === '/api/account/logout' || route === '/api/dreamiez/account/logout')) {
    clearSession(res);
  ...
  if (req.method === 'POST' && (route === '/api/account/register' || route === '/api/dreamiez/account/create')) {
    const input = await body(req);
    const email = String(input.email || '').trim().toLowerCase();
    const name = String(input.name || input.displayName || 'Dreamer').trim().slice(0, 60) || 'Dreamer';
    const password = String(input.password || '');
    ...
    const user = {
      ...
      avatar: { height: 2, build: 2, skin: 5 },
      avatar_style: String(input.avatar_style || input.avatarStyle || 'dream').slice(0, 30),
      cosmetics: [],
      streak: 1,
