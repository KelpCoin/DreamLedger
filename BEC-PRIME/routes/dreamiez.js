...
async function handle(req,res,url){
 ...
 if(req.method==='GET'&&url==='/api/dreamiez/me')return json(res,200,{id:u.id,avatar:u.avatar,cosmetics:u.cosmetics,streak:u.streak,guild:u.guild});
 ...
 if(req.method==='GET'&&url==='/api/dreamiez/leaderboard'){const list=read(USERS,[]).map(x=>({name:x.id.slice(0,8),score:(x.streak||0)*10+(x.cosmetics||[]).length*5})).sort((a,b)=>b.score-a.score);return json(res,200,list);}
 return false;
}
