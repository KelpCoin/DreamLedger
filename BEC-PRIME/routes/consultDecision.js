...
async function createCheckout(input){
  ...
  const session=await stripeRequest('POST','checkout/sessions',{
    ...
    'metadata[utm_source]':request.attribution?.utm_source||'','metadata[utm_medium]':request.attribution?.utm_medium||'','metadata[utm_campaign]':request.attribution?.utm_campaign||'','metadata[client_ref]':request.attribution?.client_ref||'',
...
async function runRefinery(request){
  ...
  if(!url||!key||!models.length)return deterministicFallback(request);
...
async function fulfill(session){
  ...
  const settlementPayload={stripe_session_id:session.id,request_id:requestIdValue,sku:C2_SKU,offer_id:C2_OFFER_ID,amount_nzd:5,currency:'NZD',livemode:Boolean(session.livemode),payment_status:session.payment_status,result_hash:resultHash,attribution:request.attribution||null};
