...
$Started = Get-Date
function Get-Json([string]$Url) { Invoke-RestMethod -Uri $Url -Method Get -Headers @{ 'Cache-Control' = 'no-cache' } }
$Result = [ordered]@{
    verifier = 'BEC-PRIME Verify-CommercialTruth v2.0'
...
try {
    ...
    $Result.gates.payment_surface_exists = ($Result.gates.stripe_configured -and $Result.gates.webhook_configured -and $Result.gates.checkout_available)
    $Result.gates.commercially_buyable = ($Result.gates.product_found -and $Result.gates.product_published -and $Result.gates.inventory_available -and $Result.gates.checkout_available -and (-not $Result.gates.approval_required))
    $required = @($Result.gates.health_ok,$Result.gates.product_found,$Result.gates.product_published,$Result.gates.inventory_available,$Result.gates.payment_surface_exists,$Result.gates.commercially_buyable)
    if (($required | Where-Object { -not $_ }).Count -eq 0) { $Result.verdict = 'CHECKOUT_READY_PAYMENT_UNPROVEN' } else { $Result.verdict = 'BLOCKED' }
} catch {
...
Write-Host "COMMERCIALLY BUYABLE: $($Result.gates.commercially_buyable)"
